...
if [ ! -f "$output_file" ]; then
    for IDX in $(seq 0 $((CHUNKS-1))); do
        TRANSFORMERS_OFFLINE=1 CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python3 videollama2/eval/run_inference_video_qa_gpt_consistency.py \
            --model-path ${CKPT} \
...
python3 videollama2/eval/eval_benchmark_5_consistency.py \
