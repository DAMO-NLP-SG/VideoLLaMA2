...
output_file=${OUTPUT_DIR}/egoschema/answers/${CKPT_NAME}_merge.csv
...
for IDX in $(seq 0 $((CHUNKS-1))); do
  TRANSFORMERS_OFFLINE=1 CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python3 videollama2/eval/run_inference_video_qa_batch.py \
      ...
      --answer-file ${OUTPUT_DIR}/egoschema/answers/${CKPT_NAME}_${CHUNKS}_${IDX}.csv \
      --num-chunks $CHUNKS \
      --chunk-idx $IDX \
      --batch-size 1 \
      --conv-mode llama_2 &
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    cat ${OUTPUT_DIR}/egoschema/answers/${CKPT_NAME}_${CHUNKS}_${IDX}.csv >> "$output_file"
done
